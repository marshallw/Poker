...
namespace Poker.HandEvaluators
{
    public class StraightHandEvaluator : BasePokerHandEvaluator
    {
        public override HandDetails GetHandRank(Hand hand)
        {
            ...
            cards.AddRange(hand.Cards.Where(_ => _.CardValue == CardValue.Ace)
                                     .Select(_ => new Card(CardValue.AceLow, _.CardSuit)));
            cards = cards.OrderBy(_ => _.CardValue)
                         .ToList();

            cards = cards.Distinct(new CardValueEqualityComparer())
                         .Aggregate(new List<Card>(), (cards, next) =>
            {
        ...
        public override bool IsHandThis(Hand hand)
        {
            ...
            cardsTransform.AddRange(cardsTransform.Where(_ => _.CardValue == CardValue.Ace)
                                                  .Select(_ => new Card(CardValue.AceLow, _.CardSuit))
                                                  .ToList());
            cardsTransform = cardsTransform.OrderBy(_ => _.CardValue)
                                           .Select((i, j) => new Card(i.CardValue - j, i.CardSuit))
                                           .ToList();

            bool result = cardsTransform.GroupBy(_ => _.CardValue)
                                        .Where(_ => _.Count() >= 5)
                                        .Any();
